...
def main():
    ...
    opts, args = getopt.getopt(sys.argv[1:], 'm:a:o:t:h',["metamap=","ann_path="," output_path=","min_threshold=", "help"])
    for opt, arg, in opts:
        if opt in ("-m","--metamap"):
            metamap_path = arg
...
def ProcessAnnotations(metamap_path, ann_path, output_path, tTest, tTreatment, tProblem):
	...
	for document in onlyFiles:
		...
		for ix, annotation in enumerate(annotationList):
			...
			labelDict[annotation.label].annotationList.append(annotation.original)

			#If metamap and annotation file agree, add to silver standard list
			if isSilver:
				labelDict[annotation.label]['silverList'].append(annotation.original)
			elif prediction == 'none':
				labelDict[annotation.label]['ambiguousList'].append(annotation.original)
	...
	for key in labelDict:	
		...
		with open(output_file,'a+', newline='') as f:
			writer = csv.writer(f)
			f.writerow(str(thresholds), key, silver, ambiguous, incorrect)
